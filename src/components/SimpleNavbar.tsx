...
import { Home, CheckSquare, BookOpen, User, Flame, Moon, Sun } from 'lucide-react';
...
const SimpleNavbar: React.FC = () => {
  ...
  const navItems: NavItem[] = [
    ...
    { path: '/assignments', label: 'Assignments', icon: <BookOpen className="w-4 h-4" /> }
  ];
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  key={item.path}
                  to={item.path}
                  className={`relative flex items-center space-x-2 px-4 py-2 rounded-lg text-sm font-medium transition-all duration-300 ${isActive
                ...
                >
                  <span className={`relative z-10 ${isActive ? 'scale-110' : ''} transition-transform duration-300`}>
                    {item.icon}
                  </span>
