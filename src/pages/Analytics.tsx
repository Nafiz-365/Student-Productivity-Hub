...
const Analytics: React.FC = () => {
    ...
    const calculateAnalytics = () => {
        ...
        const completedTasksInRange = tasksInRange.filter(
        ...
        const assignmentsInRange = assignments.filter(
            (assignment) =>
                new Date(assignment.dueDate) >= start &&
                new Date(assignment.dueDate) <= end,
        );
        const completedAssignmentsInRange = assignmentsInRange.filter(
            (a) => a.status === 'completed',
        );
        const assignmentsCompletionRate =
            assignmentsInRange.length > 0
                ? (completedAssignmentsInRange.length /
        ...
        const studySessionsInRange = studySessions.filter(
            (session) =>
                new Date(session.completedAt) >= start &&
                new Date(session.completedAt) <= end,
        );
        const totalStudyTime = studySessionsInRange.reduce(
            (total, session) => total + session.duration,
            0,
        );
        const averageSessionLength =
            studySessionsInRange.length > 0
        ...
        return {
            tasks: {
                ...
                pending: tasksInRange.filter((t) => t.status === 'pending')
                    .length,
            },
            assignments: {
                ...
                pending: assignmentsInRange.filter(
                    (a) => a.status === 'pending',
                ).length,
            },
            study: {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div className="card p-6 border-l-4 border-l-emerald-500 glass-panel group hover:-translate-y-1 transition-transform duration-300">
                    ...
                    <p className="text-xs text-gray-500 mt-2 font-medium">
                        {analytics.tasks.completed} out of{' '}
                        {analytics.tasks.total} tasks
                    </p>
                </div>
                    ...
                    <div className="flex justify-between items-start mb-4">
                        <div>
                            ...
                            <h3 className="text-3xl font-bold text-gray-900 dark:text-white mt-1 group-hover:text-violet-600 transition-colors">
                                {analytics.assignments.averageGrade > 0
                                    ? `${analytics.assignments.averageGrade.toFixed(1)}%`
                                    : 'N/A'}
                            </h3>
                        </div>
                    ...
                    <div className="flex justify-between items-start mb-4">
                        <div>
                            ...
                            <h3 className="text-3xl font-bold text-gray-900 dark:text-white mt-1 group-hover:text-amber-600 transition-colors">
                                {analytics.study.averageSessionLength.toFixed(
                                    0,
                                )}
                                m
                            </h3>
                ...
                <div className="lg:col-span-2 card p-6 glass-panel">
                    <div className="flex items-center justify-between mb-6">
                        <h3 className="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
                            <Clock className="w-5 h-5 text-indigo-500" /> Study
                            Activity
                        </h3>
                            ...
                            options={{
                                ...
                                plugins: {
                                    ...
                                    tooltip: {
                                        ...
                                        callbacks: {
                                            label: (context) =>
                                                `${context.parsed.y} minutes focus`,
                                        },
                                    },
                                },
                                scales: {
