...
type Action =
    ...
    | {
          ...
          payload: { id: string; assignment: Partial<Assignment> };
      }
    ...
    | {
          type: 'UPDATE_CLASS_SCHEDULE';
          payload: { id: string; schedule: Partial<ClassSchedule> };
      }
...
const appReducer = (state: any, action: Action) => {
    switch (action.type) {
        ...
        case 'UPDATE_CLASS_SCHEDULE':
            return {
                ...state,
                classSchedule: state.classSchedule.map(
        ...
        case 'DELETE_CLASS_SCHEDULE':
            return {
                ...
                classSchedule: state.classSchedule.filter(
                    (schedule: ClassSchedule) => schedule.id !== action.payload,
                ),
            };
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({
    ...
    useEffect(() => {
        const loadData = () => {
            try {
                ...
                const assignments = localStorage.getItem(
                    STORAGE_KEYS.ASSIGNMENTS,
                );
                const classSchedule = localStorage.getItem(
                    STORAGE_KEYS.CLASS_SCHEDULE,
                );
                const studySessions = localStorage.getItem(
                ...
                if (assignments)
                    dispatch({
                        ...
                        payload: JSON.parse(assignments),
                    });
                if (classSchedule)
                    dispatch({
                        type: 'SET_CLASS_SCHEDULE',
                        payload: JSON.parse(classSchedule),
                    });
                if (studySessions)
                    dispatch({
                        type: 'SET_STUDY_SESSIONS',
                        payload: JSON.parse(studySessions),
                    });
    ...
    useEffect(() => {
        try {
            ...
            localStorage.setItem(
                STORAGE_KEYS.CLASS_SCHEDULE,
                JSON.stringify(state.classSchedule),
            );
            localStorage.setItem(
                STORAGE_KEYS.STUDY_SESSIONS,
    ...
    const addClassSchedule = (
        schedule: Omit<ClassSchedule, 'id' | 'userId'>,
    ) => {
